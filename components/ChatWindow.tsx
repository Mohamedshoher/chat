              ...
              <div className="absolute top-full left-0 mt-2 w-64 bg-white/90 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl p-4 z-50 animate-in fade-in zoom-in duration-200">
                ...
                <div className="space-y-4 text-slate-800">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm">تدمير ذاتي للرسائل</span>
                      <button
                        onClick={() => onUpdateSettings({
                          autoDeleteEnabled: !chat.settings.autoDeleteEnabled,
                          autoDeleteDuration: chat.settings.autoDeleteEnabled ? undefined : 1440
                        })}
                    ...
                    {chat.settings.autoDeleteEnabled && (
                      <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 animate-in slide-in-from-top-2">
                        <span className="text-xs text-slate-500 block mb-2">حذف الرسائل تلقائياً بعد:</span>
                        <div className="grid grid-cols-3 gap-1">
                          {[
                            { label: 'ساعة', val: 60 },
                            { label: '٢٤ ساعة', val: 1440 },
                            { label: '٧ أيام', val: 10080 }
                          ].map((option) => (
                            <button
                              key={option.val}
